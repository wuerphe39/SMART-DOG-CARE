...
- 복합 이상 감지: 여러 지표가 동시에 이상일 때 더 신뢰성 있는 경보 발령
...
[베이스라인 관리자]
  ...
  +-- 베이스라인 B: 보호자 없을 때 행동 통계 (시간대별 평균/표준편차)
        |
[이상 감지 엔진]  <-- Z-score 기반 통계적 이상 탐지 + 복합 이상 감지 (scipy)
        |
[케어 실행 모듈]
...
YOLOv8 커스텀 학습 모델로 강아지의 행동 상태를 감지한다.

행동 상태: `resting` / `active` / `near_bowl` / `absent`
...
librosa 에너지 임계값 기반으로 짖음 유무를 판단한다.
- 짖음 감지 시 보호자 알림 발송
- 진정 사운드 자동 재생 (선택)
...
│   ├── owner_detector.py      # 보호자 존재 감지 (YOLOv8 COCO person)
│   ├── audio_detector.py      # 짖음 감지 (librosa)
│   └── models/
│
├── analysis/
│   ├── baseline_manager.py    # 베이스라인 A/B 구축 및 갱신
│   ├── anomaly_detector.py    # Z-score 단일·복합 이상 감지
...
### Phase 3 — 분석 엔진 개발 (1개월)

- [ ] 베이스라인 관리자 구현 (A/B 분리, rolling 갱신)
- [ ] Z-score 기반 단일 이상 감지 엔진 구현
- [ ] 복합 이상 감지 로직 구현 (Multi-signal Anomaly)
- [ ] 컨텍스트 인사이트 생성 모듈 구현
- [ ] 개인화 알림 메시지 포맷 구현
- [ ] SQLite 연동 및 통합 테스트
